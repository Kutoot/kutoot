    ...
    public function summary(Request $request, MerchantLocation $merchantLocation): JsonResponse
    {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'nonRedeemedVisitors' => max(0, $totalVisitors - $redeemedVisitors),
                'conversionRate' => $totalVisitors > 0 ? round(($redeemedVisitors / $totalVisitors) * 100, 2) : 0,
                'redemptionRate' => $totalVisitors > 0 ? round(($redeemedVisitors / $totalVisitors) * 100, 2) : 0,
                ...
                'peakHours' => $peakHours,
                'metricDescriptions' => [
                    'totalSales' => 'Sum of all original bill amounts from successful transactions in the selected period.',
                    'totalDiscount' => 'Total discount value applied across all successful transactions.',
                    'totalVisitors' => 'Count of unique users (customers) who made at least one successful transaction.',
                    'offerViews' => 'Estimated offer/deal impressions based on visitor traffic (visitors × 2.5).',
                    'redeemedVisitors' => 'Unique users who redeemed at least one coupon/deal during their transaction.',
                    'nonRedeemedVisitors' => 'Unique visitors who transacted but did not redeem any coupon (totalVisitors - redeemedVisitors).',
                    'conversionRate' => 'Percentage of visitors who redeemed a coupon: (redeemedVisitors / totalVisitors) × 100.',
                    'redemptionRate' => 'Same as conversion rate — percentage of visitors who used a deal.',
                    'repeatVisitors' => 'Users who transacted more than once in the selected period.',
                    'activeCampaigns' => 'Number of currently active deals/coupons for this store.',
                    'commissionPercent' => 'Commission percentage configured for this store location.',
                    'sellerBalance' => 'Net seller earnings: totalSales - totalDiscount - totalCommission. Uses monthly summary if available for current month.',
                    'successTransactions' => 'Total number of successful (paid/completed) transactions.',
                    'failedTransactions' => 'Total number of failed transactions.',
                    'totalCommissionPaid' => 'Total commission amount deducted from successful transactions.',
                    'peakDays' => 'Top 3 days of the week with highest transaction count.',
                    'peakHours' => 'Top time ranges with highest transaction activity.',
                ],
            ],
        ]);
    }
